...
const TYPE_META: Record<ClinicianNotificationType, { icon: LucideIcon; color: string }> = {
  assignment: { icon: Users, color: "from-[oklch(0.62_0.14_235)] to-[oklch(0.48_0.13_245)]" },
  task: { icon: ClipboardList, color: "from-[oklch(0.72_0.14_75)] to-[oklch(0.58_0.13_55)]" },
  ...
  security: { icon: KeyRound, color: "from-[oklch(0.68_0.14_158)] to-[oklch(0.52_0.12_160)]" },
};
...
export default function ClinicianNotificationsPage() {
  const { state, markNotificationRead, markAllNotificationsRead } = useClinicianStore();
  ...
  return (
    <>
      <PageHeader
        ...
        title="Stay informed"
        description="Patient assignments, consent changes, task pings, and security alerts — all categorized."
      />
...
function NotificationRow({ n, onOpen }: { n: ClinicianNotification; onOpen: () => void }) {
